...
const handleMatchesExport = async () => {
  ...
  try {
    ...
    const html = `
      <html>
        ...
        <body>
          <h1>SpotSync Match Results Export</h1>
          <p>Generated By: ${firstName} ${lastName} (${email})</p>
            ...
            <tbody>
              ${matchesData
                ...
                <tr>
                  ...
                  <td>${match.foundItem?.itemId || "-"}</td>
                  <td>${match.foundItem?.itemName || "-"}</td>
                  <td>${match.lostItem?.itemId || "-"}</td>
                  <td>${match.lostItem?.itemName || "-"}</td>
                  <td>${match.foundItem?.category || "-"}</td>
                  <td>${match.scores?.overallScore || "-"}</td>
                  <td>${match.scores?.nameScore || "-"}</td>
                  <td>${match.scores?.descriptionScore || "-"}</td>
                  <td>${match.scores?.imageScore || "-"}</td>
                  <td>${match.scores?.locationScore || "-"}</td>
                  <td>${
    ...
    a.href = url;
    a.download = "SpotSync_Matches_Report.html"; 
    a.click();
    URL.revokeObjectURL(url);

    setAlert({ message: "Matches exported successfully!", type: "success" });

  } catch (err) {
    console.error(err);
    setAlert({ message: "Failed to export matches.", type: "error" });
  }
      ...
      <div style={styles.foundItemBody}>
        <div style={styles.settingsContainer}>
          ...
          <div style={styles.otherSettings}>
            ...
            <div
              style={{ ...styles.otherSettingsP, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
              onClick={!exporting ? handleMatchesExport : null}
            >
