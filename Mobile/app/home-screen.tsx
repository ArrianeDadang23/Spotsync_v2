...
  const [alert, setAlert] = useState({ message: '', type: 'success', visible: false });
  ...
  const isProfileIncomplete = userData
    ? requiredFields.some((field) => {
        ...
        return value === undefined || value === null || (typeof value === 'string' && value.trim() === "") || (typeof value === 'object' && value !== null && Object.keys(value).length === 0);
      })
    : true; 
...
            <Text style={styles.infoPanelText}>Please complete your profile to report items.</Text>
...
          <TouchableOpacity
            style={[styles.actionButton, isProfileIncomplete && styles.actionButtonDisabled]} 
            onPress={() => {
                if (!isProfileIncomplete) {
                    router.push(`/UserLostProcedureScreen`);
                } else {
                    Alert.alert("Profile Required", "Please complete your profile before reporting a lost item.");
                }
            }}
...
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.actionButton, { marginLeft: 15 }, isProfileIncomplete && styles.actionButtonDisabled]} 
            onPress={() => {
                if (!isProfileIncomplete) {
                    router.push(`/UserFoundProcedureScreen`);
                } else {
                    Alert.alert("Profile Required", "Please complete your profile before reporting a found item.");
                }
            }}
            disabled={isProfileIncomplete} 
          >
